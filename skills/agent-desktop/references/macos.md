...
- `hover`, `mouse-move`, and `mouse-click` accept the opt-in, best-effort `--background` mode ([background-input.md](background-input.md)): the event goes to one exact window's process with window-routing fields and a window-local location, posted through SkyLight, after a target-only window-server focus record; a focus guard restores the user's frontmost app if the target activates itself, but leaves a switch to any other app alone. The cursor never moves; focus preservation is best effort. Results are `delivered_unverified` and report `focus_change`, `layers`, `degraded`, and `focus_guard`; confirm the effect with `snapshot`. `AGENT_DESKTOP_BG_LAYERS` (`route,skylight,activate,guard,primer`, or `none`) narrows the recipe for diagnosis; see `docs/solutions/best-practices/background-pointer-delivery-2026-09-23.md`. Sandboxed apps may still drop these events.
- `press --background --window-id w-N` and `type --background` (a ref, or `--window-id`) post key events to that window's process after target-only records make it the key window inside its own process (activation record plus yabai's make-key pair), with an `SLSEventAuthenticationMessage` attached for Chromium. The app is not activated and no key is mapped to a menu or AX action. Text is Unicode per character; results are `delivered_unverified`, confirmed by `snapshot`. `AGENT_DESKTOP_BG_LAYERS` also accepts `auth` and `keywindow`; see `docs/solutions/best-practices/background-keyboard-delivery-2026-09-24.md`.
