...
`--background` (macOS only; `hover`, `mouse-move`, `mouse-click`) posts the pointer event directly to the process that owns one exact window with `CGEventPostToPid`. The user's cursor never moves, the app is not activated or raised, and keyboard focus is untouched, so it works in the default headless mode and is rejected with `--headed`.
...
- **Result.** Success carries `disposition: { delivery: "delivered_unverified", retry: "unsafe" }` and `background: { pid, window_id, focus_change, frontmost_pid_before?, frontmost_pid_after? }`. `focus_change` is `unchanged`, `changed` (with a top-level `warning`), or `unknown`. A repeated hover is harmless, but the contract still says `unsafe`: observe the effect with `snapshot` instead of retrying.
