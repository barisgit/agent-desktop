...
agent-desktop type @s8f3k2p9:e7 "hello" --background # Type into a background window's element as key events (macOS)
...
`press` and `type` also accept `--background` (macOS): keys go to one exact window's process after that window is made key inside its app, without activating it. `press` needs `--window-id` (not `--app`); `type` takes the window from its ref and tries an accessibility focus on the element first without depending on it. No key is mapped to a menu item or AX action. The result is `delivered_unverified` like the background pointer; confirm the text with `snapshot` and do not retry blindly.
